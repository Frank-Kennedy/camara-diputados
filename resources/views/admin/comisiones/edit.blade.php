...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-parlamento-azul">✏️ Editar Comisión</h1>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
            <form method="POST" action="{{ route('admin.comisiones.update', $comision->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name_es" value="{{ old('name_es', $comision->name_es) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="name_fr" value="{{ old('name_fr', $comision->name_fr) }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-parlamento-azul">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    <div>
                        ...
                        <input type="text" name="name_pt" value="{{ old('name_pt', $comision->name_pt) }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-parlamento-azul">
                    </div>
                    <div>
                        <label class="block text-gray-700 font-medium mb-2">Nombre (Inglés)</label>
                        <input type="text" name="name_en" value="{{ old('name_en', $comision->name_en) }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-parlamento-azul">
                    </div>
                </div>
                ...
                <div class="mt-4">
                    ...
                    <textarea name="description_es" rows="4" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-parlamento-azul">{{ old('description_es', $comision->description_es) }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                    <div>
                        ...
                        <textarea name="description_fr" rows="3"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-parlamento-azul">{{ old('description_fr', $comision->description_fr) }}</textarea>
                    </div>
                    <div>
                        <label class="block text-gray-700 font-medium mb-2">Descripción (Portugués)</label>
                        <textarea name="description_pt" rows="3"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-parlamento-azul">{{ old('description_pt', $comision->description_pt) }}</textarea>
                    </div>
                    <div>
                        <label class="block text-gray-700 font-medium mb-2">Descripción (Inglés)</label>
                        <textarea name="description_en" rows="3"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-parlamento-azul">{{ old('description_en', $comision->description_en) }}</textarea>
                    </div>
                </div>
                ...
                <div class="mt-4">
                    ...
                    <input type="file" name="image" accept="image/*"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-parlamento-azul">
                    <p class="text-xs text-gray-500 mt-1">Formatos: JPG, PNG, GIF. Máx: 5MB. Dejar vacío para mantener la imagen actual.</p>
                    @error('image') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div class="mt-4">
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" {{ $comision->is_active ? 'checked' : '' }}
                ...
                <div class="mt-6 flex gap-3">
                    <button type="submit"
                        ...
                        <i class="fas fa-save mr-2"></i> Actualizar Comisión
                    </button>
