                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @if($documento->has_pdf)
                        <a href="{{ route('transparencia.download', $documento->id) }}" 
                           class="flex items-center gap-4 p-4 bg-red-50 border border-red-200 rounded-xl hover:bg-red-100 transition group">
                    ...
                    @if($documento->has_excel)
                        <a href="{{ route('transparencia.download-excel', $documento->id) }}" 
                           class="flex items-center gap-4 p-4 bg-green-50 border border-green-200 rounded-xl hover:bg-green-100 transition group">
                            <div class="text-3xl text-green-500 group-hover:scale-110 transition">
                    ...
                    @if(!$documento->has_pdf && !$documento->has_excel)
                        <p class="text-gray-500 col-span-2 text-center py-8">
                            <i class="fas fa-file text-4xl block mb-2 opacity-50"></i>
