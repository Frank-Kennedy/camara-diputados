...
@section('title', 'Crear Documento de Transparencia')

@section('content')
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-parlamento-azul">📄 Crear Documento</h1>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
            <form method="POST" action="{{ route('admin.transparencia.store') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="category" required
                            ...
                            <option value="presupuesto" {{ old('category') == 'presupuesto' ? 'selected' : '' }}>💰 Presupuesto</option>
                            <option value="informe_gestion" {{ old('category') == 'informe_gestion' ? 'selected' : '' }}>📊 Informe de Gestión</option>
                            <option value="rendicion_cuentas" {{ old('category') == 'rendicion_cuentas' ? 'selected' : '' }}>⚖️ Rendición de Cuentas</option>
                            <option value="contrataciones" {{ old('category') == 'contrataciones' ? 'selected' : '' }}>📄 Contrataciones</option>
                            <option value="planificacion" {{ old('category') == 'planificacion' ? 'selected' : '' }}>📅 Planificación</option>
                        </select>
                    </div>
                    <div>
                ...
                <div class="mt-4">
                    ...
                    <input type="date" name="publication_date" value="{{ old('publication_date', date('Y-m-d')) }}" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-parlamento-azul">
                </div>
                ...
                <div class="mt-4">
                    ...
                    <input type="file" name="file_pdf" accept=".pdf"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-parlamento-azul">
                    <p class="text-xs text-gray-500 mt-1">Máx: 20MB</p>
                </div>
